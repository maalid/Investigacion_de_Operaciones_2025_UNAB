...
# |     |PROGRAMADAS  | MAX.        | DE PRODUCCION* | DE ALMACENAJE* |  
...
# Producción Mes 2 y ocupada mes 1: M ya que por lógica está condición es imposible de ocurrir.
...
# Producción Mes 4 y ocupada mes 1: M ya que por lógica está condición es imposible de ocurrir.
# Producción Mes 4 y ocupada mes 2: M ya que por lógica está condición es imposible de ocurrir.
# Producción Mes 4 y ocupada mes 3: M ya que por lógica está condición es imposible de ocurrir.
...
# x21 : Cantidad de motores de turbina producidos el mes 2, para instalar en el mes 1.(Condición imposible de ocurrir)
...
# x31 : Cantidad de motores de turbina producidos el mes 3, para instalar en el mes 1.(Condición imposible de ocurrir)
# x32 : Cantidad de motores de turbina producidos el mes 3, para instalar en el mes 2.(Condición imposible de ocurrir)
...
# x41 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 1.(Condición imposible de ocurrir)
# x42 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 2.(Condición imposible de ocurrir)
# x43 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 3.(Condición imposible de ocurrir)
...
# En resumen, definimos las 20 variables anteriores mediante una sola expresión. 
#
# Para i=1,2,3,4 y j=1,2,3,4,5 sea: 

#	xij: Cantidad de motores de turbinas producidos en el mes i, que se van a instalar en el mes j.

# Funcion Objetivo.(Se asigna el costo M a los escenarios imposibles de que ocurran).
...
#                       x11 + x12 + x13 + x14 + x15  = 25 (Restricción de producción máxima mes 1).
#                       x21 + x22 + x23 + x24 + x25  = 35 (Restricción de producción máxima mes 2).
#                       x31 + x32 + x33 + x34 + x35  = 30 (Restricción de producción máxima mes 3).
#                       x41 + x42 + x43 + x44 + x45  = 10 (Restricción de producción máxima mes 4).
...
# El PLAN ÓPTIMO de producción que es capaz de minimizar los costos de ALMACENAMIENTO y,
...
# (Estos motores de turbina al ir directamente a  un nodo ficticio, no es necesario fabricarlos,
# porque no serán ocupados).
...
# El costo mínimo asociado a este  Plan de producción es de  77,3 millones de dólares:
...
#Para obtener la mejor programación del número de turbinas y que a la vez se minimicen los costos
#de producción y almacenamiento, el Gerente de Producción debe seguir el siguiente plan:
#
#  En el mes 1, se deben producir 10 motores de turbina e instalarlos dentro del mismo mes 1.
#  El costo total de esta producción es de = 10.8 millones de dólares.

#  En el mes 1, se deben producir 10 motores de turbina, almacenarlos e instalarlos dentro del
#  mes 2. El costo total de esta producción y almacenamiento es de = 10.95 millones de dólares.

#  En el mes 1, se deben producir 5 motores de turbina, almacenarlos e instalarlos dentro del
#  mes 4. El costo total de esta producción y almacenamiento es de = 5.625 millones de dólares.

#  En el mes 2, se deben producir 5 motores de turbina e instalarlos dentro del mismo mes 2.
#  El costo total de esta producción y almacenamiento es de= 5.55 millones de dólares.

# En el mes 3, debe producir 25 motores de turbina e instalarlos dentro del mismo mes 3.
# El costo total de esta producción es de = 27.5 millones de dólares.

# En el mes 3, debe producir 5 motores de turbina e instalarlos dentro del mismo mes 4.
# El costo total de esta producción es de = 5.575 millones de dólares.

# En el mes 4, debe producir 10 motores de turbina e instalarlos dentro del mismo mes 4.
# El costo total de esta producción es de = 11.3 millones de d ólares.

# Lo que se traduce a un costo total de Producción y Almacenamiento de:  77.3 millones de dólares. 
# Dicho valor es el costo mínimo asociado al plan de transporte mencionado. 

# Es importante decir, que dentro del mes 2, existen 30 motores de turbinas que no son fabricados,
# ya que no existe demanda para dichos motores.
#En un futuro se deberá evaluar si se pueden vender esos motores o si se puede aumentar la demanda de motores.
...
# Al ser un problema de transbordo, se debe traspasar a un problema de transporte de la 
# siguiente manera:

# Paso 1
# Establecer la cantidad de reserva.
...
# La cantidad de reserva es: R = 5.000.

# PASO 2. 
# A la oferta de cada origen y la demanda de cada destino se le suma la cantidad
# de reserva R = 5.000.
# Las ofertas quedan así: A = 7.000 y B = 8.000.
# Las demandas quedan así: C = 6.500 , D = 6.700 , E = 6.800.

# PASO 3. 
# Colocar todos los destinos como orígenes asignándole una oferta igual a
# la cantidad de reserva R. Quedan con una oferta de 5.000 unidades.

# PASO 4. 
# Colocar todos los orígenes como destinos asignándole una demanda igual 
# a la cantidad de reserva R. Quedan con una demanda de 5.000 unidades.

# PASO 5. 
# Establecer las condiciones de equilibrio entre oferta y demanda tal 
# como se hace en el modelo del transporte.

# La tabla de ofertas, demandas y costos queda de la siguiente manera:
...
# Definición de las variables de decisión (del problema equilibrado)
...
# En resumen definimos las 49 variables anteriores mediante una sola expresión. 
#
# Para i=1,2,3,4,5,6,7 y j=1,2,3,4,5,6,7 sea: 

#	xij: cantidad de máquinas de coser producidas semanalmente en el destino i, que son enviadas al destino j.
...
# El plan óptimo de transporte que logra minimizar los costos, es el siguiente:
...
# Para que Electromaquinas Ltda.pueda reducir al mínimo sus costos semanales asociados
# a la distribución de sus máquinas de coser, deberá seguir el siguiente Plan Óptimo:
#
#
#Enviar su Producción semanal desde la planta A, de 2.000 máquinas de coser al almacen
#temporal F.

# Enviar su producción semanal desde la planta B, de 3.000 máquinas de coser al almacen
#temporal F.

# Distribuir a los centros de distribución desde el almacen temporal de la siguiente forma:
...
# Lo que se traduce a un costo total de transporte de 80.300 unidades monetarias. 
# Dicho valor es el costo mínimo asociado a este plan de transporte.
...
# Al ser un problema de asignación, éste se debe equilibrar,
# por lo tanto se debe crear un nodo ficticio de un quinto tipo
# de nado (OFERTA), para satisfacer los 5 nadadores disponibles (DEMANDA)

# El problema planteado quedaría de la siguiente forma:

#  _______________________________________________________
# |               |           NADADORES                   |
# |   TIPO DE     |_______________________________________|           
# |     NADO      | CARL  | CHRIS | DAVID | TONY  | KEN   |  
...
# En resumen, definimos las 25 variables anteriores mediante una sola expresión. 
#
# Para i=1,2,3,4,5 y j=1,2,3,4,5 sea: 
...
#                    x51 + x52 + x53 + x54 + x55  = 1 (Restricción del tipo de nadp FICTICIO).
#                    x11 + x21 + x31 + x41 + x51  = 1 (Restricción del tipo CARL).
#                    x12 + x22 + x32 + x42 + x52  = 1 (Restricción del tipo CHRIS).
#                    x13 + x23 + x33 + x43 + x53  = 1 (Restricción del tipo DAVID).
#                    x14 + x24 + x34 + x44 + x54  = 1 (Restricción del tipo TONY).
#                    x15 + x25 + x35 + x45 + x55  = 1 (Restricción del tipo KEN).
#                            
...
#               X55 = 0
#
# El tiempo mínimo asociado a este plan óptimo de asignación es de  126,2 segundos:
...
# Para minimizar los tiempos y asignar de la mejor manera los competidores en la prueba
# de 200 metros de relevo combinado, el entrenador del equipo de natación debe seguir el siguiente plan:
#
#       - Asignar a CARL en la prueba de nado tipo LIBRE con un tiempo de 29.2 segundos.
#       - Asignar a CHRIS en la prueba de nado tipo MARIPOSA con un tiempo de 28.5 segundos.
#       - Asignar a DAVID en la prueba de nado tipo DORSO con un tiempo de 33.8 segundos.
#       - Asignar a TONY en la prueba de nado tipo PECHO con un tiempo de 34.7 segundos.
#       - No asignar a KEN a ninguna de las pruebas de nado.

# Lo que se traduce en un tiempo total de la prueba de:  126.2 segundos. 
# Dicho valor es el mínimo tiempo que se puede demorar en la prueba de nado.
