...
# mundo. La última etapa del proceso de producción consiste en fabricar las turbinas de jet e
# instalarlas -una operación muy rápida- en la estructura del avión terminado. La compañía tiene
# varios contratos de trabajo que la obligan a entregar un número considerable de aviones en un futuro
# cercano y en este momento debe programar la producción de motores de turbina para los próximo
# cuatro meses.

# En la segunda columna de la tabla se indica la cantidad de motores que debe estar lista para su
# instalación a fin de cumplir con las fechas de entrega contratadas. De ella se desprende que el
# número acumulado de motores que deben producirse al final de los meses 1, 2, 3 y 4 debe ser por lo
# menos de 10, 25, 50 y 70 unidades, respectivamente.
...
# mes | instalaciones programadas | poduccion maxima | costo unit. produccion | costo unit. almacenaje
# 1   |          20               |       25         |          1.08          |         0.015
# 2   |          15               |       35         |          1.11          |         0.015
# 3   |          25               |       30         |          1.10          |         0.015
# 4   |          20               |       10         |          1.12          |
...
# al formular este problema con metodo de trasporte se obtiene la siguiente tabla:

#              |                  DESTINO                        |
#              --------------------------------------------------
#         mes  |     1    |    2    |     3    |     4   |   5   |  OFERTA
#    ----------------------------------------------------------------------
#    O  |  1   |   1.08   |   1.095 |   1.11   |   1.125 |   0   |     25
#    R  |      |          |         |          |         |       |
#    I  |  2   |     M    |  1.11   |   1.125  |  1.14   |   0   |     35 
#    G  |      |          |         |          |         |       | 
#    E  |  3   |     M    |    M    |   1.1    |   1.115 |   0   |     30
#    N  |      |          |         |          |         |       |
#       |  4   |     M    |    M    |     M    |   1.13  |   0   |     10
#    ----------------------------------------------------------------------
#      DEMANDA |    10    |    15   |    25    |    20   |  30   |    100

# En la tabla se agrega un nodo 5 en destino, los costos se traducen en: 
...
# x21 : Cantidad de motores de turbina producidos el mes 2, para instalar en el mes 1 (imposible)
...
# x31 : Cantidad de motores de turbina producidos el mes 3, para instalar en el mes 1 (imposible)
# x32 : Cantidad de motores de turbina producidos el mes 3, para instalar en el mes 2 (imposible)
...
# x41 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 1 (imposible)
# x42 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 2 (imposible)
# x43 : Cantidad de motores de turbina producidos el mes 4, para instalar en el mes 3 (imposible)
...
# Podemos definir las 20 variables anteriores en una sola expresión

# Entonces, para i=1,2,3,4 y j=1,2,3,4,5 sea: 

#	xij: cantidad de motores de turbinas producidas en el mes (i), que corresponde instalar en el mes (j)
...
# En base a la definición obtenida para las variables de decisión y la infomacion en el problema
# matemáticamente el modelo de Programación Lineal a resolver es:
...
#  b) Obtenga una solution optima.


# El plan óptimo de producción que minimiza los costos de fabricación y almacenamiento es:
...
# (nodo ficticio, por lo tanto no es necesario fabricarlos,
# X25 = 30
...
# al problema de transbordo, se debe traspasar a un problema de transporte

# 1. Establecer la cantidad de reserva
...
#    la cantidad de reserva R = 5.000.

# 2. A la oferta de cada origen y la demanda de cada destino se le suma la cantidad de reserva R = 5.000.
#    ofertas : A = 7.000 y B = 8.000.
#    demandas: C = 6.500 , D = 6.700 , E = 6.800.

# 3. todos los destinos y orígenes se les asigna una oferta igual a la cantidad de reserva R.
#    oferta de 5.000 unidades.

# 4. todos los orígenes y destinos se les asigna una demanda igual a la cantidad de reserva R
#    demanda de 5.000 unidades.

# 5. Establecer las condiciones de equilibrio entre oferta y demanda, como en modelo de transporte.

# Tabla de ofertas, demandas y costos:
...
#   R  |     |       |       |       |       |       |       |       |          
#   i  |  B  |   M   |   0   |   M   |   M   |   M   |   3   |   11  |   8000   
#   G  |     |       |       |       |       |       |       |       |          
#   E  |  C  |   M   |   M   |   0   |   M   |   M   |   M   |   M   |   5000 
#   N  |     |       |       |       |       |       |       |       |          
#   E  |  D  |   M   |   M   |   M   |   0   |   M   |   M   |   M   |   5000    
#   S  |     |       |       |       |       |       |       |       |          
...
#  podemos definir las 49 variables resumudamente para i=1,2,3,4,5,6,7 y j=1,2,3,4,5,6,7 sea: 
...
# la solucion óptima de transporte que minimiza los costos, es el siguiente:
...
# Lse traduce a un costo total de tranporte de 80.300 unidades monetarias.Siendo el costo minimo asociado a este modelo de transporte 
...
# El entrenador de un equipo de natación debe asignar competidores para la prueba de 200 metros de relevo combinado que
# irá a las Olimpiadas Juveniles. Como muchos de sus mejores nadadores son rápidos en más de un estilo, no es fácil
...
# El entrenador quiere determinar cómo asignar cuatro nadadores a los cuatro estilos de nado para minimizar
# la suma de los mejores tiempos correspondientes.
...
# Problema de asignación, por tanto se debe equilibrar creaando un nodo ficticio de un quinto estilo de nado (oferta)
# para satisfacer los 5 nadadores disponibles (demanda)

# La formulacion obtenida es la sigueinte :
...
# podemos definir las 25 variables anteriores resumidas en:
# i=1,2,3,4,5 y j=1,2,3,4,5 sea: 
...
# la solucion óptima de asignación que minimiza la suma de los tiempos de los nadadores es:
...
# Entonces el tiempo mínimo asociado a este modelo de asignación es de  126,2 segundos:
# Z* =  126,2 segundos.
...
# El entrenador del equipo de natación pueda minimizar los tiempos y asignar de la mejor manera
# los competidores en la prueba de 200 metros de relevo combinado, debe seguir el siguiente plan:
...
# finalmente se obtiene un tiempo total de la prueba de:  126.2 segundos. Que es el tiempo minimo en realizar 
# la prueba de nado los nadadores  CARL, CHRIS, DAVID y TONY
